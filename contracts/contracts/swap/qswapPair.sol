...
contract QswapConstantProductPair {
    ...
    constructor(
        address _tokenX,
        uint256 _amountTokenX,
        address _tokenY,
        uint256 _amountTokenY,
        uint256 _fee,
        address tokenBalanceUpdater,
        ...
        _tokenBalanceUpdater.updateBalance(
            _tokenX,
            tx.origin,
            address(this),
            _amountTokenX
        ...
        uint256 liquidityTokenAmount = qswapSqrt.sqrt(
            _amountTokenX * _amountTokenY
        );
    ...
    function swap(
        ...
        uint256 denominator = scaledIn + scaledSwap;
    ...
    function addLiquidity(uint256 amount, bool isReverse) external {
        ...
        uint256 liquidityMinted = liquidityToken.totalSupply() -
            liquidityBefore;
        _updateReserves();
    ...
    function _getLiquidityOptions(
        bool isReverse
    )
        private
        view
        returns (uint256 x, uint256 y, address xAddress, address yAddress)
    {
        if (isReverse) {
            x = _reserveX;
            y = _reserveY;
    ...
    function _updateReserves() private {
        _reserveX = _tokenBalanceUpdater.getTokenBalance(
            address(tokenX),
            address(this)
        );
        _reserveY = _tokenBalanceUpdater.getTokenBalance(
            address(tokenY),
            address(this)
        );
    }
