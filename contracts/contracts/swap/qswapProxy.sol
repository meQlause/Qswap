...
contract QswapProxy {
    ...
    function createPair(
        ...
        address tokenY,
        uint256 amountTokenY,
        uint256 fee
    ) external returns (address pair) {
        require(tokenX != tokenY, "D0");
        require(getPair[tokenX][tokenY].pairAddress == address(0), "D1");
        address liquidityToken = address(
            new QswapTokenCreator(
                0,
                string(
                    abi.encodePacked(
                        QswapTokenCreator(tokenX).name(),
                        QswapTokenCreator(tokenY).name()
                    )
                ),
                "QSLT",
                true,
        ...
        pair = address(
            new QswapConstantProductPair(
                tokenX,
                amountTokenX,
                tokenY,
                amountTokenY,
                fee,
                address(_tokenBalanceUpdaterContract),
                liquidityToken
            )
        );
    ...
    function swap(
        ...
        QswapConstantProductPair pairContract = QswapConstantProductPair(
            pairToUse
        );
    ...
    function removeLiquidity(
        ...
        QswapConstantProductPair pairContract = QswapConstantProductPair(
            getLTPair[liquidityTokenAddress]
        );
        pairContract.removeLiquidity(amount);
        return true;
    }

    function _setIsReverse(
        address tokenX,
        address tokenY
    ) private view returns (bool isReverse, address pairToUse) {
